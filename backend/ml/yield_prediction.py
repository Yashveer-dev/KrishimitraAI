...
class CropYieldPredictor:
    ...
    def train_xgboost(self, X: np.ndarray, y: np.ndarray) -> Dict[str, Any]:
        ...
        grid_search = GridSearchCV(
            xgb_reg, param_grid, cv=3, scoring='neg_mean_squared_error', n_jobs=-1
        )
        grid_search.fit(X_train, y_train)
        ...
        metrics = {
            ...
            'mae': mean_absolute_error(y_test, y_pred),
            'r2_score': r2_score(y_test, y_pred),
            'feature_importance': {k: float(v) for k, v in zip(self.feature_columns, self.xgb_model.feature_importances_)}
        }
